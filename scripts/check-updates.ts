#!/usr/bin/env tsx
/**
 * Lesotho Law MCP — Data Freshness Checker
 *
...
async function checkPortal(url: string): Promise<boolean> {
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: { 'User-Agent': '@ansvar/lesotho-law-mcp/1.0 (data-freshness-check)' },
    });
...
async function main(): Promise<void> {
  console.log('Lesotho Law MCP — Data Freshness Check');
