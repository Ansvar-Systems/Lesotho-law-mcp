#!/usr/bin/env tsx
/**
 * Drift detection for Lesotho Law MCP.
 *
 * Checks if upstream Lesotho Law content has changed since last ingestion.
...
async function main(): Promise<void> {
  console.log('Lesotho Law MCP — Drift Detection');
  console.log('================================\n');
  ...
  for (const hash of fixture.provisions) {
    ...
    try {
      const response = await fetch(hash.upstream_url, {
        headers: { 'User-Agent': 'lesotho-law-mcp/1.0 drift-detect' },
      });
