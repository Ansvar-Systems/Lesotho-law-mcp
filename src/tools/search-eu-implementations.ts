/**
 * search_eu_implementations — Search EU directives/regulations that have Lesotho implementing legislation.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_from?: number;
  year_to?: number;
  has_lesotho_implementation?: boolean;
  limit?: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      ed.title,
      ed.short_name,
      COUNT(DISTINCT er.document_id) as lesotho_statute_count
  ...
  if (input.has_lesotho_implementation) {
    sql += ' HAVING lesotho_statute_count > 0';
  }
