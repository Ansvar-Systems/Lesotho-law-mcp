/**
 * get_lesotho_implementations — Find Lesotho statutes implementing a specific EU directive/regulation
...
export interface GetLesothoImplementationsInput {
  eu_document_id: string;
  primary_only?: boolean;
  in_force_only?: boolean;
}

export interface LesothoImplementationResult {
  document_id: string;
...
export async function getLesothoImplementations(
  db: InstanceType<typeof Database>,
  input: GetLesothoImplementationsInput,
): Promise<ToolResponse<LesothoImplementationResult[]>> {
  try {
  ...
  const rows = db.prepare(sql).all(...params) as LesothoImplementationResult[];
